...
- `cityclassifiers/models/heads/*` for embedding/sequence heads.
- `cityclassifiers/models/backbones/*` for end-to-end backbone+head wrappers.
- `cityclassifiers/models/tasks/*` for new losses or task-specific helpers.
...
Example paths:
- `cityclassifiers/models/heads/my_new_head.py`
- `cityclassifiers/models/backbones/my_new_backbone.py`

## 3. Export it from package init
1. Update `cityclassifiers/models/heads/__init__.py` or `cityclassifiers/models/backbones/__init__.py`.
2. Export the class in `__all__` if used.

## 4. Register the model ID
1. Add a stable ID in `cityclassifiers/models/registry.py`.
...
`cityclassifiers/models/factory.py` already uses signature-based kwarg filtering via `build_model_with_filtered_kwargs(...)`.

## 6. If you add a new loss
1. Implement it under `cityclassifiers/models/tasks/`.
2. Export it from `cityclassifiers/models/tasks/__init__.py`.
3. Add handling in `cityclassifiers/models/factory.py::build_criterion(...)`.
...
- Prefer package paths (`cityclassifiers.models.*`) for all new logic.

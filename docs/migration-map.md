...
|---|---|---|
| `train.py` | embeddings/e2e training script with setup + loop | `cityclassifiers/cli/train_embeddings.py` + `cityclassifiers/training/*` + `cityclassifiers/models/factory.py` |
| `train_features.py` | feature-sequence training script with setup + loop | `cityclassifiers/cli/train_features.py` + `cityclassifiers/training/*` + `cityclassifiers/models/factory.py` |
| `inference.py` | pipelines + helper functions + model loading | `cityclassifiers/cli/infer.py` + `cityclassifiers/inference/pipeline.py` + `cityclassifiers/inference/postprocess.py` |
...
| `dataset.py` | embedding dataset + validation subset | `cityclassifiers/data/embeddings.py` |
| `sequence_dataset.py` | sequence dataset + collate + validation subset | `cityclassifiers/data/sequences.py` |
| `image_dataset.py` | image dataset + collate helpers | `cityclassifiers/data/images.py` |
| `model.py` | embedding predictor head | `cityclassifiers/models/heads/predictor.py` |
| `head_model.py` | sequence head architecture | `cityclassifiers/models/heads/sequence_head.py` |
...
- Target:
  - `cityclassifiers/training/bootstrap.py` (already started)
...
- Target:
  ...
  - `cityclassifiers/training/engine.py` (or callback submodule)
  - `cityclassifiers/utils/logging.py`
...
- Target:
  ...
  - `cityclassifiers/config/serialization.py` (optional)

## 2.4 Inference internals
- Current:
  - `inference.py`: preprocess helpers + `BasePipeline` + many derived pipelines
- Target:
  - `cityclassifiers/inference/pipeline.py` (core classes)
  - `cityclassifiers/inference/postprocess.py`
  - `cityclassifiers/inference/preprocess.py`
...
- Move dataset/collate/dataloader setup from scripts into `cityclassifiers/data/`.
- Keep wrapper functions in scripts temporarily forwarding to new modules.

## Slice F: Engine extraction (Phase 5)
- Move shared loop mechanics, checkpoint IO, optimizer/scheduler setup to `cityclassifiers/training/`.
...
2. Config, data, model factory, training engine, and inference pipeline live under `cityclassifiers/`.
